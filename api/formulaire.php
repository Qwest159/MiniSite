...
$contacts = json_decode(file_get_contents('../storage/contact.json'), true); // provoque un null => recupere les elements du fichier json



$data = json_decode(file_get_contents('php://input'), true); // recupere les name des input

$contacts[] =  $data; // avoie les donnée rajouter a contact(contact fait reference au json avec les données deja existante)
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!isset($args["erreurs"])) { // si tableau erreur ne contient rien,
        ...
        file_put_contents('../storage/contact.json', json_encode($args["valeurNetoyee"][$nomChamp]));
